...
function loadRuntimeContext(rootUrl = new URL('../..', import.meta.url), options = {}) {
  ...
  source += options.exposeEngine === true ? `
    globalThis.__integrationEngine = {
      ...
      setState(s) {
        campaignMagic=MAGIC_RULES.settings(s.campaignMagic);recipeState=RECIPE_RULES.state(s.recipeState);
      ...
      state() { return {
        campaignMagic,recipeState,chars,journal,items:itemsDB,spells:spellsDB,abilities:abilitiesDB,races:racesDB,
      ...
      itemsApi: {
        ...
        body:itemPlayerBodyHTML, tooltip:bagItemTooltipHTML, inventoryActions:inventoryBagActionsHTML,
        equipmentDetails:equipmentItemDetailsHTML, equipmentCandidate:equipmentCandidateCardHTML, equipmentCurrent:equipmentCurrentCardHTML,
        equipmentFacts:equipmentItemFacts, combatInspect:combatInspectItem, materialInspect, triggerExplain:itemTriggerExplain,
      ...
      recipesApi:{outputStatus:recipeOutputStatus,outputHTML:recipeOutputBodyHTML,legacyPlan:craftPlanFor,legacyCommit:commitCraftPlan,rows:gameRecipeRows,groups:()=>recipeSearchRows(''),of:gameRecipeOf,status:recipeStatus,prepare:gameRecipePrepare,commitGame:gameRecipeCommit,defaults:recipeDefaultChoices,workbench:recipeWorkbenchHTML,state:()=>recipeState,setState:s=>recipeState=RECIPE_RULES.state(s),select:recipeSelect,failPersistence:()=>{runScheduledSave=async()=>false;},local:ITEM_RECIPES_V45,load:()=>bg3CatalogEnsureAsset('recipes'),display:bg3RecipeDisplayItem,inputLabel:bg3RecipeInputLabel,available:bg3AvailableIds,sources:bg3RecipeProgramSourcesFor,plan:bg3RecipeProgramPlanFor,commit:bg3RecipeProgramCommit,card:bg3RecipeCardHTML,panel:bg3RecipesPanelHTML,requirements:craftRequirementsOf,summary:id=>bg3Catalog.summaries.get(id)},
      ...
      craftPlanFor,commitCraftPlan,inventoryItemQty,combatSnapshotOf,combatSyncChanges,gameContextFor,resetCustomCharacterForCombat,
