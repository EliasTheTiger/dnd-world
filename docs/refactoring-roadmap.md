...
| R3 Ruleset | Done | Редакция была неявной. Зафиксирован 2014 core и BG3 Standard extension. | `ruleset-registry.js`, `data/rulesets/manifest.json`; envelope хранит refs. | `ruleset-registry.test.cjs` | Unknown/mismatch definition не исполняется. |
...
| R6 Spell/feature catalogs | Gate done; content blocked | «Максимум» не имел source/license/expected census. Введён import/completeness gate. | `catalog-governance.js`, `data/catalogs/source-manifest.json`; автоматической content migration нет. | `catalog-governance.test.cjs` | Код завершён. Наполнение заблокировано до утверждённого легального ID-корпуса; это внешний вход, а не повод фабриковать данные. |
...
5. `approved-2014-corpus`: после предоставления источника заполнить source manifest, expected IDs/hash и импортировать spell/features с per-definition executable tests.
