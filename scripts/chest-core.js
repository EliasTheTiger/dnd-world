(function chestCoreModule(root,factory){
  ...
  const ACTIONS=Object.freeze(['inspect','check','open','pick','disarm','destroy','claim']);
  ...
  const RARITY_ALIASES=Object.freeze({
    ...
    rare:'rare','редкий':'rare','редкая':'rare','very rare':'very-rare','veryrare':'very-rare','very_rare':'very-rare','very-rare':'very-rare','очень редкий':'very-rare','очень редкая':'very-rare',
  ...
  function normalizeLoot(value){
    ...
    return {schemaVersion:LOOT_SCHEMA,status:['empty','draft','approved','claimed'].includes(row.status)?row.status:'empty',seed:text(row.seed),mode:['auto','empty','currency','items','mixed','fixed'].includes(row.mode)?row.mode:'auto',allowIncompatible:bool(row.allowIncompatible),
  ...
  function normalizeTrap(value,variant){
    const row=value&&typeof value==='object'?value:{},enabled=row.enabled==null?variant==='trapped':bool(row.enabled);
    return {enabled,type:text(row.type)||'needle',detectionDC:int(row.detectionDC,0,40,enabled?13:0),disarmDC:int(row.disarmDC,0,40,enabled?13:0),saveDC:int(row.saveDC,0,40,enabled?12:0),saveAbility:['str','dex','con','int','wis','cha'].includes(row.saveAbility)?row.saveAbility:'dex',saveEffect:row.saveEffect==='none'?'none':'half',damageFormula:text(row.damageFormula)||'1d6',damageType:text(row.damageType)||'piercing',detected:bool(row.detected),disarmed:bool(row.disarmed),triggered:bool(row.triggered)};
  }
  ...
  function validateLootDraft(chest,catalog,draft){
    ...
    const totals=new Map(),masterChoice=chest.variant==='story'||container&&container.loot.allowIncompatible;for(const entry of normalized.items){const item=map.get(entry.itemId);totals.set(entry.itemId,(totals.get(entry.itemId)||0)+entry.qty);if(!item)issues.push('UNKNOWN_ITEM_ID:'+entry.itemId);else if((!itemAllowed(item,{level,rarityCap,context})&&!masterChoice)||item.impossible||item.kind==='creature')issues.push('INCOMPATIBLE_ITEM:'+entry.itemId);if(item&&item.unique&&totals.get(entry.itemId)>1)issues.push('DUPLICATE_UNIQUE_ITEM:'+entry.itemId);}
  ...
  function generateLoot(chestInput,catalog,seedOverride){
    ...
    if(chest.container.loot.status==='claimed'||chest.container.loot.claimedByActorId)return {ok:false,code:'LOOT_ALREADY_CLAIMED'};
    if(chest.container.rewardMode==='fixed'||chest.variant==='story')return {ok:false,code:'FIXED_LOOT_IS_MANUAL'};
    const container=chest.container,seed=text(seedOverride)||text(container.loot.seed)||chest.id+':loot',random=seededRandom(seed),mode=generationMode(container,random),draft={items:[],currency:normalizeMoney(null),notes:''};
    if(mode==='items'||mode==='mixed'){const catalogIds=catalogMap(catalog),missingTableIds=container.lootTable.map(row=>row.itemId).filter(itemId=>!catalogIds.has(itemId));if(missingTableIds.length)return {ok:false,code:'UNKNOWN_LOOT_TABLE_ITEM',issues:missingTableIds.map(itemId=>'UNKNOWN_ITEM_ID:'+itemId)};}
    ...
    if(mode==='items'||mode==='mixed'||mode==='fixed'){
      ...
      for(let index=0;index<count;index++){let available=candidates.filter(row=>!row.item.unique&&row.item.stackable||!selected.has(row.item.id)),picked=weightedPick(available,random);if(!picked)return {ok:false,code:'NOT_ENOUGH_DISTINCT_LOOT',issues:['NOT_ENOUGH_DISTINCT_LOOT']};const existing=draft.items.find(row=>row.itemId===picked.item.id);if(existing&&picked.item.stackable)existing.qty++;else draft.items.push({itemId:picked.item.id,qty:1});selected.add(picked.item.id);}
    }
    const validation=validateLootDraft(chest,catalog,draft);chest.container.loot={schemaVersion:LOOT_SCHEMA,status:'draft',seed,mode,allowIncompatible:false,draft:validation.draft,approved:null,validation:{ok:validation.ok,issues:validation.issues},claimedByActorId:null};
  ...
  function replaceLootDraft(chestInput,catalog,draft){const chest=normalizeChest(chestInput),loot=lootOf(chest);if(!loot)return {ok:false,code:'CREATURE_IS_NOT_CONTAINER'};if(loot.status==='claimed'||loot.claimedByActorId)return {ok:false,code:'LOOT_ALREADY_CLAIMED'};const validation=validateLootDraft(chest,catalog,draft);loot.status='draft';loot.draft=validation.draft;loot.approved=null;loot.validation={ok:validation.ok,issues:validation.issues};loot.claimedByActorId=null;return {ok:validation.ok,chest,loot:clone(loot),issues:validation.issues};}
  function approveLoot(chestInput,catalog){const chest=normalizeChest(chestInput),loot=lootOf(chest);if(!loot)return {ok:false,code:'CREATURE_IS_NOT_CONTAINER'};if(loot.status==='claimed'||loot.claimedByActorId)return {ok:false,code:'LOOT_ALREADY_CLAIMED'};const validation=validateLootDraft(chest,catalog,loot.draft);loot.validation={ok:validation.ok,issues:validation.issues};if(!validation.ok)return {ok:false,code:'LOOT_VALIDATION_FAILED',issues:validation.issues,chest};loot.status='approved';loot.approved=clone(validation.draft);return {ok:true,chest,loot:clone(loot)};}
  ...
  function evaluateAction(chestInput,action,context){
    ...
    if(action==='claim'){
      if(!container||['decoy','creature'].includes(chest.variant))return actionReject('NO_CONTAINER_LOOT','В этом объекте нет добычи.');
      if(container.loot.status==='claimed'||container.loot.claimedByActorId)return actionReject('LOOT_ALREADY_CLAIMED','Добыча уже выдана персонажу.');
      if(!['opened','destroyed'].includes(chest.state))return actionReject('CHEST_NOT_OPEN','Сначала откройте или разрушьте сундук.');
      if(container.loot.status!=='approved'||!container.loot.approved)return actionReject('LOOT_NOT_APPROVED','Содержимое ещё не утверждено мастером.');
      return {allowed:true,reasonCode:'ALLOWED',explanation:'Забрать утверждённые предметы и монеты выбранным персонажем.'};
    }
    ...
    if(action==='destroy'){if(chest.kind==='creature')return actionReject('CREATURE_USES_COMBAT','Это существо. Раскройте его и разрешайте атаки в разделе «Бой».');return physical&&physical.durability.current>0?{allowed:true,reasonCode:'ALLOWED',explanation:'Требуется ввести фактический урон объекту.'}:actionReject('CHEST_NO_DURABILITY','Объект уже не имеет прочности.');}
    ...
    if(action==='pick'){
      if(container.lock.type==='none')return actionReject('CHEST_HAS_NO_LOCK','У сундука нет замка.');if(container.lock.opened||chest.state==='forced')return actionReject('CHEST_ALREADY_UNLOCKED','Замок уже вскрыт.');if(container.lock.type==='story'&&container.lock.dc<=0)return actionReject('STORY_LOCK_REQUIRES_KEY','Сюжетный замок нельзя вскрыть обычной проверкой.');if(context.actorHasTools===false)return actionReject('THIEVES_TOOLS_REQUIRED','Нужны воровские инструменты в инвентаре персонажа.');return {allowed:true,reasonCode:'ALLOWED',explanation:'Требуется фактический d20 Ловкости с воровскими инструментами.'};
    }
    if(action==='disarm'){
      if(!trap.enabled)return actionReject('CHEST_HAS_NO_TRAP','У сундука нет ловушки.');if(!trap.detected)return actionReject('TRAP_NOT_DETECTED','Сначала ловушку нужно обнаружить проверкой.');if(trap.disarmed)return actionReject('TRAP_ALREADY_DISARMED','Ловушка уже обезврежена.');if(trap.triggered)return actionReject('TRAP_ALREADY_TRIGGERED','Ловушка уже сработала.');if(context.actorHasTools===false)return actionReject('THIEVES_TOOLS_REQUIRED','Нужны воровские инструменты в инвентаре персонажа.');return {allowed:true,reasonCode:'ALLOWED',explanation:'Требуется фактический d20 Ловкости с воровскими инструментами.'};
    }
    if(action==='open'){
  ...
  function applyAction(chestInput,action,resolution){
    const chest=normalizeChest(chestInput),check=evaluateAction(chest,action,{actorId:text(resolution&&resolution.actorId),actorAlive:resolution&&resolution.actorAlive!==false});if(!check.allowed)return {ok:false,code:check.reasonCode,reason:check.explanation,chest};resolution=resolution||{};const actorId=text(resolution.actorId),physical=physicalOf(chest),container=containerOf(chest),total=Number(resolution.total);
    if(['check','pick','disarm'].includes(action)&&(resolution.total==null||resolution.total===''||!Number.isFinite(total)))return {ok:false,code:'CHECK_RESULT_REQUIRED',reason:'Введите результат проверки.',chest};
    if(action==='destroy'&&(resolution.damage==null||resolution.damage===''||!Number.isSafeInteger(Number(resolution.damage))||Number(resolution.damage)<0))return {ok:false,code:'DAMAGE_RESULT_REQUIRED',reason:'Введите фактический урон.',chest};
    if(action==='claim')return actionOutcome(chest,action,true,'LOOT_READY_TO_CLAIM','Содержимое передано выбранному персонажу.',{actorId,lootReady:true});
    ...
    if(action==='open'){
      ...
      chest.state='opened';const triggered=physical.trap.enabled&&!physical.trap.disarmed&&!physical.trap.triggered;if(triggered)physical.trap.triggered=true;
      if(chest.variant==='decoy')return actionOutcome(chest,action,true,'DECOY_OPENED','Ложный сундук открыт; содержимого нет.',{actorId,trapTriggered:triggered});
      if(chest.variant==='creature'){chest.creature.revealed=true;return actionOutcome(chest,action,true,'CREATURE_RELEASED','Из контейнера появляется игровое существо.',{actorId,creatureInstanceId:chest.creature.instanceId,requiresCombatEngine:true,trapTriggered:triggered});}
      return actionOutcome(chest,action,true,triggered?'CHEST_OPENED_TRAP_TRIGGERED':'CHEST_OPENED',triggered?'Сундук открыт; ловушка сработала и требует структурированного разрешения.':'Сундук открыт. Выберите получателя и заберите содержимое.',{actorId,trapTriggered:triggered,lootReady:!!(container.loot&&container.loot.approved)});
    }
