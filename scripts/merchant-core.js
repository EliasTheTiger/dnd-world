(function (root, factory) {
  ...
  const RARITY_ORDER = Object.freeze({
    common:0,'обычный':0,'обычная':0,uncommon:1,'необычный':1,'необычная':1,rare:2,'редкий':2,'редкая':2,
    'very rare':3,'very-rare':3,'veryrare':3,'очень редкий':3,'очень редкая':3,legendary:4,'легендарный':4,'легендарная':4,artifact:5,'артефакт':5,
  });
  const FAILURE_MESSAGES = Object.freeze({
    'invalid-request':'Неверные данные сделки.',
    'unknown-template':'Шаблон торговца не найден.',
    'unknown-item':'Предмет не найден в действующей базе предметов.',
    'merchant-restricted':'Торговля недоступна при текущих условиях региона или сюжета.',
    ...
    'merchant-item-blocked':'Торговец отказывается покупать или продавать этот предмет.',
  ...
  function removeCharacterItem(character,removals){
    for(const removal of removals){const index=(character.inventory||[]).findIndex(row=>row.id===removal.entryId&&row.itemId===removal.itemId),row=character.inventory[index];if(!row||row.qty<removal.qty)throw coded('stale-state');row.qty-=removal.qty;if(row.qty===0){character.inventory.splice(index,1);for(const slot of Object.keys(character.equipment||{}))if(character.equipment[slot]===row.id)delete character.equipment[slot];}}
  }
  ...
  class MerchantService {
    ...
    restock(instance,context){
      ...
      const targets=new Map();for(const rule of source.initialStock)for(const item of this._candidates(source,rule,context||instance).slice(0,rule.limit))targets.set(item.id,(targets.get(item.id)||0)+rule.quantity);
      let added=0;for(const [itemId,quantity] of targets){const existing=stockEntry(instance,itemId);if(!existing){addMerchantStock(instance,itemId,quantity);added+=quantity;}else if(source.restockRules.mode==='restore-to-target'&&existing.quantity<quantity){added+=quantity-existing.quantity;existing.quantity=quantity;}}
    ...
    quote(instance,character,itemId,side,quantity){
      ...
      if(!['purchase','sale'].includes(side))return failResult('invalid-request');
      if(!restrictionCheck(source.restrictions,instance).ok)return failResult('merchant-restricted');
      if(side==='sale'&&!itemMatches(item,source.buyCategories))return failResult('merchant-does-not-buy-category');
    ...
    _guard(character,itemId,quantity,context,entryId){const rows=(character.inventory||[]).filter(row=>row.itemId===itemId&&(!entryId||row.id===entryId));let left=quantity,removals=[];for(const row of rows){const take=Math.min(left,Math.max(0,Number(row.qty)||0));if(take===Number(row.qty)&&(character.inventory||[]).some(child=>child.inside===row.id))return failResult('inventory-guard',{message:'Сначала выньте содержимое из продаваемого контейнера.'});if(take)removals.push({entryId:row.id,itemId,qty:take});left-=take;if(!left)break;}if(left)return failResult('seller-item-missing');const checked=this.inventoryGuard(character,removals,{context});return checked&&checked.ok!==false?{ok:true,removals}:failResult('inventory-guard',{details:checked});}
    buy(input){
      const value=input||{};return this._withInstance(value.instance,async()=>{try{
        ...
        const consequence=()=>{if(value.instance.revision!==revision)throw coded('stale-state');const beforeInventory=inventorySnapshot(value.character),beforeStock=clone(value.instance.inventory),journalLength=this.journal.length,updatedAt=value.instance.updatedAt;try{const fresh=stockEntry(value.instance,item.id);if(!fresh||fresh.quantity<qty||fresh.available===false)throw coded('stale-state');fresh.quantity-=qty;addCharacterItem(value.character,item.id,qty,this.idFactory);value.instance.revision++;value.instance.updatedAt=this.clock();this.journal.push(seed);return()=>{inventoryRestore(value.character,beforeInventory);value.instance.inventory=clone(beforeStock);value.instance.revision=revision;value.instance.updatedAt=updatedAt;this.journal.length=journalLength;};}catch(error){inventoryRestore(value.character,beforeInventory);value.instance.inventory=clone(beforeStock);value.instance.revision=revision;value.instance.updatedAt=updatedAt;this.journal.length=journalLength;throw error;}};
    ...
    sell(input){
      const value=input||{};return this._withInstance(value.instance,async()=>{try{
        ...
        const guarded=this._guard(value.character,item.id,qty,'merchant-sale',value.entryId);if(!guarded.ok)return guarded;const quote=this.quote(value.instance,value.character,item.id,'sale',qty);if(!quote.ok)return quote;
        ...
        const consequence=()=>{if(value.instance.revision!==revision)throw coded('stale-state');const beforeInventory=inventorySnapshot(value.character),beforeStock=clone(value.instance.inventory),journalLength=this.journal.length,updatedAt=value.instance.updatedAt;try{if(characterQuantity(value.character,item.id)<qty)throw coded('stale-state');const freshGuard=this._guard(value.character,item.id,qty,'merchant-sale-commit',value.entryId);if(!freshGuard.ok)throw coded('inventory-guard',freshGuard.message,freshGuard.details);removeCharacterItem(value.character,freshGuard.removals);addMerchantStock(value.instance,item.id,qty);if(shortfall)value.instance.overdraftMinor=(BigInt(value.instance.overdraftMinor||0)+shortfall).toString();value.instance.revision++;value.instance.updatedAt=this.clock();this.journal.push(seed);return()=>{inventoryRestore(value.character,beforeInventory);value.instance.inventory=clone(beforeStock);value.instance.overdraftMinor=beforeOverdraft;value.instance.revision=revision;value.instance.updatedAt=updatedAt;this.journal.length=journalLength;};}catch(error){inventoryRestore(value.character,beforeInventory);value.instance.inventory=clone(beforeStock);value.instance.overdraftMinor=beforeOverdraft;value.instance.revision=revision;value.instance.updatedAt=updatedAt;this.journal.length=journalLength;throw error;}};
